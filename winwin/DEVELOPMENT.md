...
└── src/
    ...
    ├── layout.rs       # 配置(基準位置・幅・高さ)から矩形を求める計算
...
| `layout.rs` | 長さの読み書き、半分・4 分の 1・3 分の 1 の矩形、他のモニター座標、DPI 拡大、作業領域からはみ出さないこと |
| `config.rs` | 初期値の妥当性と読み書きの往復、手書きファイルの読み込み、以前の版の `name`・`offset_x`・`offset_y` を読み捨てること、不正な値の拒否、同じショートカットの項目を順番どおりに読むこと、ファイルの作成と上書き |
