...
└── src/
    ...
    ├── layout.rs       # 配置(基準位置・幅・高さ・ずらし幅)から矩形を求める計算
...
`win/` 以外の 4 つは Win32 に依存しないので、どのプラットフォームでもテストできます。挙動を決めるロジックはなるべく
...
- 同じ `keys` を 2 つの項目に使うと読み込みエラーになります
...
`GetMessageW` で眠っていて、タイマーやポーリングはありません。`MOD_NOREPEAT` を付けているので、押しっぱなしでも
1 回しか届きません。
...
`cargo test` で走るのは、Win32 に依存しない 4 つのモジュールの単体テストです(Windows 以外でも走ります)。

| モジュール | 確かめていること |
|---|---|
| `hotkey.rs` | 表記の読み書き、すべての仮想キーが書いた表記から読み戻せること、ショートカットにできない組み合わせの拒否 |
| `layout.rs` | 長さの読み書き、半分・4 分の 1・3 分の 1 の矩形、他のモニター座標、DPI 拡大、ずらし幅、作業領域からはみ出さないこと |
| `config.rs` | 初期値の妥当性と読み書きの往復、手書きファイルの読み込み、不正な値と重複ショートカットの拒否、ファイルの作成と上書き |
| `draft.rs` | 設定画面の 1 行と設定の往復、保存時の検証メッセージ、複製、一覧の表示文字列、hotkey コントロールの値の変換 |
