...
右下の「表示」で、設定画面とトレイメニューをライト・ダークに固定できます(既定は Windows の設定に合わせる)。設定画面
...
ダイアログを開いたまま他のアプリに切り替えると、そちらでは普通に入力できる / ダークモードでトレイの右クリックメニューが暗い色になる / 設定画面の「表示」をライト・ダークに変えると画面がすぐ切り替わり、
