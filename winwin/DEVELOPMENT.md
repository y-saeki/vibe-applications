...
リソースファイルを持たず、実行時に `icon.rs` がピクセルを描いて作ります。設定画面のタイトルバーとタスクバーのボタンにも、
同じ絵を大小 2 つのサイズで描いて `WM_SETICON` で付けています(`settings_ui.rs` の `adopt_window`。閉じるボタンの
サブクラス化と同じ場所で、ウィンドウが見つからなければ既定のアイコンのままです)。exe ファイル自体のアイコンは Windows 既定のものです。
