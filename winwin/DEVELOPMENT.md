...
しているので、配置どおりの大きさになります。`WS_EX_TOOLWINDOW` なのでタスクバーと Alt+Tab に出ず、常駐側が設定画面を
前面に出すとき(`settings.rs` の `bring_forward`)もこのウィンドウは選びません。Alt+F4 で閉じようとすると、設定画面に
伝えて閉じます。設定画面を閉じるとプロセスごと消えます。
...
- テスト用ウィンドウの見た目(すりガラス・単色)、ショートカットで動くこと、ドラッグとサイズ変更
...
すりガラスのウィンドウが開き、保存前に変えたショートカットで(設定画面にフォーカスがあっても)そのウィンドウだけが動く。
切り替えショートカットも順に切り替わり、`1/12` のような小さい配置でもその大きさになる。「閉じる」と Alt+F4 で閉じる /
