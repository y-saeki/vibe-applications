//! Persistent application state: the two texts plus every user setting,
...
/// cannot be parsed is moved aside as `state.json.broken` so the texts are
...
pub fn load(path: &Path) -> io::Result<State> {
    ...
    match serde_json::from_str(&raw) {
        ...
        Err(err) => {
            ...
            eprintln!(
                "diffpad: {} could not be parsed ({err}); moving it to {}",
                path.display(),
                broken.display()
            );
...
/// the previous file or the new one, never a truncated text.
...
mod tests {
    ...
    fn a_field_an_older_version_never_wrote_falls_back_to_its_default() {
        let dir = TempDir::new();
        fs::write(dir.state(), r#"{"textA":"昔のファイル"}"#).expect("the file is written");
    ...
    fn saving_over_an_existing_file_replaces_it() {
        ...
        assert_eq!(
            load(&dir.state()).expect("the state is read back").text_a,
            "二回目"
        );
    }
}
