...
pub fn install(app: &AppHandle) -> tauri::Result<()> {
    ...
    let about = AboutMetadata {
        name: Some("diffpad".into()),
        version: Some(app.package_info().version.to_string()),
        ..Default::default()
    };
    let app_menu = SubmenuBuilder::new(app, "diffpad")
        .about(Some(about))
        .separator()
        ...
        .show_all()
        .separator()
        .item(&item("quit", "Quit diffpad", "CmdOrCtrl+Q")?)
        .build()?;
    ...
    // The same keys VS Code's diff editor uses; `src/commands.ts` binds them
    // in the webview on Windows.
    let go = SubmenuBuilder::new(app, "Go")
        .item(&item("next_diff", "Next Difference", "F7")?)
        .item(&item("previous_diff", "Previous Difference", "Shift+F7")?)
        .build()?;
    ...
    let menu = MenuBuilder::new(app)
        .items(&[&app_menu, &edit, &view, &go, &text])
        .build()?;
    app.set_menu(menu)?;
    ...
    app.on_menu_event(|app, event| {
        ...
        if FORWARDED_IDS.contains(&id) {
            if let Err(err) = app.emit("menu", id) {
                eprintln!("diffpad: failed to forward menu event {id}: {err}");
            }
        }
        #[cfg(debug_assertions)]
        if id == "devtools" {
