// Persistent state: the two texts plus every setting. Mirrors `State` in
// src-tauri/src/state.rs. Settings changes go through `Store.set`; the texts
// live in the editors and are pulled in lazily when a save happens.

import { invoke } from '@tauri-apps/api/core'

export type Theme = 'system' | 'light' | 'dark'
/** How a changed chunk is marked: whole lines only, or the characters within them too. */
export type DiffMode = 'line' | 'char'

export interface State {
  /** The left pane, which the right one is compared against. */
  textA: string
  /** The right pane. */
  textB: string
  diffMode: DiffMode
...
/** The two texts, as the editors hand them back at save time. */
...
export class Store {
  ...
  constructor(initial: State) {
    ...
    this.textProvider = () => ({ textA: this.current.textA, textB: this.current.textB })
  }
  ...
  /** The editors register themselves here so saves read the live documents. */
  ...
  private async save(): Promise<void> {
    ...
    this.dirty = false
    const { textA, textB } = this.textProvider()
    this.current.textA = textA
    this.current.textB = textB
    const snapshot: State = { ...this.current }
    this.inflight = invoke<void>('save_state', { state: snapshot })
      .catch((err: unknown) => {
        console.error('diffpad: failed to save state', err)
        this.dirty = true
      })
      .finally(() => {
        this.inflight = null
