...
export function createCommands(actions: CommandActions, platform: string): Command[] {
  ...
  const commands: Command[] = [
    ...
    // inside the editors, and a window-wide shortcut would take Ctrl+Z away from
    // the text fields in the preferences panel.
    { id: 'undo', title: '元に戻す', run: actions.undo },
    { id: 'redo', title: 'やり直す', run: actions.redo },
    // F7 and Shift+F7 are what VS Code's diff editor uses. The editors' own
    // keymap leaves the function keys alone, so nothing competes for them.
    { id: 'next_diff', title: '次の差異', keys: 'F7', run: actions.nextDiff },
    { id: 'previous_diff', title: '前の差異', keys: 'Shift+F7', run: actions.previousDiff },
