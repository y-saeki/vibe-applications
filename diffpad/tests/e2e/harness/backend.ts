// The fake Rust side of diffpad, injected into the page before the app's own
...
const harness = window.__diffpad
...
function handle(cmd: string, args: unknown): unknown {
  ...
  switch (cmd) {
    ...
    case 'plugin:window|is_fullscreen':
      return fullscreen
    // diffpad never maximizes the window itself; it only asks before it
    // records a new size.
    ...
    default:
      throw new Error(`diffpad test: unexpected command ${cmd}`)
  }
}
