...
|---|---|
| An entry in the command table (`src/commands.ts`), or a shortcut or its default key (`src/shortcuts.ts`) | `tests/e2e/shortcuts.spec.ts` (the Windows path) **and** `tests/e2e/menu.spec.ts` (the macOS path) |
