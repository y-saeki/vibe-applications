...
描かずに済ませられるのは、そもそも横に溢れる場所がないからです。下書きは全部の行が折り返しますし
(`EditorView.lineWrapping`)、環境設定パネルはラベル列が固定で入力欄が残りを取るので、最小の
ウィンドウ幅でもはみ出しません。これは「いまのところそうなっている」ことなので、E2E テストで
固定してあります。`tests/e2e/editing.spec.ts` が下書きに、`tests/e2e/preferences.spec.ts` が
