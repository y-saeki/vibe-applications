...
ページの一部です(作りは「タイトルバー」の節)。帯はペインのバーと同じ `--bg` で塗り、間に線は引きません。
タイトルバーとバーが 1 本に見えます。

配色は「紙とインク」です。ウィンドウ全体(タイトルバーとペインのバーを含む)が 1 枚の紙 `--bg` で、
文字と線は同じ色相の墨色(`--fg` / `--muted` / `--border` など)で描きます。部品は輪郭ではなく
一段沈んだ面 `--control-bg`(ホバーで `--control-hover`、押下で `--control-active`)で紙から区別し、
環境設定パネル・ドロップダウンの一覧・検索パネルは紙より手前の面 `--panel` に載せます。
操作に反応する色は `--accent` の 1 色だけで、押されたままのボタン(常に手前に表示、Aa / .*)は
`--accent-soft` の下地に `--accent` の印です。検索の一致だけは蛍光ペンの黄色
(`--editor-search-match` / `-selected`)で、選択範囲(`--editor-selection`、青)と色相で見分けます。
ダークは同じ系統の色で明暗だけを反転させたもので、影だけは黒のままです。

エディタ本文のフォントサイズは設定項目なので、このトークンには含めません。CodeMirror が自分で描く部分の色は
`src/editor-theme.ts` にあります。キャレット・選択範囲・検索の一致・入力候補の一覧はライトとダークの両方で
パレットから塗り、ダークではさらにエディタの背景と構文ハイライトを持ちます。構文ハイライトの色だけは直値で
(パレットとは別の体系なので意図的にそうしています)、ライトは CodeMirror の既定の配色のままです。
...
入力済みに見えます。入力欄の面(`--control-bg`)に対して、ライト・ダークとも 3:1 を超えるように揃えてあります。
...
11 項目は「編集」「表示」の 2 グループに分けてあります。`<fieldset>` + `<legend>` で、既定の枠線と余白は
...
条件で、どちらのテーマでも同じ作りです。台は部品の面 `--control-bg`、オフのノブはパネルと同じ手前の面
`--panel` に `--border` の輪郭で、紙の上にパネルが載っているのと同じ関係になります。
