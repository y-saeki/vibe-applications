...
開くとウィンドウの幅を 2 倍に、閉じると半分にします(`src/main.ts` の `changePanes`、Tauri の `set_size`)。
...
ペインを作り直すのと、ウィンドウの幅が実際に変わるのは同時になりません。`set_size` はプラットフォームに頼む
操作で、返ってくるのは少し後です。どちらが先でも、エディタは一度だけ「これから持つのではない幅」で組まれます
— 古いウィンドウに 2 ペイン、または新しいウィンドウに 1 ペイン — その間だけ折り返しがそこで起きるので、本文が
ガタッと動いて見えます([#99](https://github.com/y-saeki/vibe-applications/issues/99))。

そこで、作り直す前に `:root` へ `data-resizing="grow"`(閉じるときは `"shrink"`)を付け、バーの行とペインの箱の
幅を `style.css` が同じ倍率(`200%` / `50%`)にします。ペインは最初から最終的な幅で組まれ、ウィンドウの外へ
はみ出した分は `body` の `overflow: hidden` が切ります。ウィンドウが追いついたら属性を外しますが、そのときの
`100%` は押さえていた幅と同じなので、外しても何も動きません。追いついたかどうかはページ自身の `resize` で
判断します。プラットフォームが受け付けなかった大きさでは `resize` が来ないため、`RESIZE_SETTLE_MS` で諦めます。

幅を押さえないのはフルスクリーンと最大化中だけです。ウィンドウが広がらず、ペインは今ある幅を分ける、つまり
本当に幅が変わるからです。それを訊くのに await が入るので、作り直す直前にもう一度「まだその変更が必要か」を
確かめます。キーを押しっぱなしにしたときに、開いているペインをもう一度開き、ウィンドウをもう一度倍にしない
ためです。

`tests/e2e/compare.spec.ts` が、押さえている間のバーの行・ペインの箱・本文の幅と、外れた後のそれを見ています。
ハーネスではウィンドウが実際には変わらないので、ビューポートを手で広げて「ウィンドウが追いついた」ところを
作ります。
