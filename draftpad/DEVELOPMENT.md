...
画面の右に収まるかどうかは見ません。はみ出したらユーザーが動かせます。大きさを変えないのはフルスクリーン
(macOS の緑ボタンで入るもの)だけで、その中で左右に分けます。

画面全体を覆っているだけのウィンドウ(Windows の最大化、macOS のズーム、手で画面いっぱいに広げたもの)は
ほかと同じく 2 倍・半分にします。画面の左半分に置いたウィンドウでペインを開くと画面いっぱいになり、macOS は
これをズーム中(`isMaximized` が真)と答えることがあります。ここを大きさを変えない側に入れていたころは、閉じても
左半分に戻りませんでした。最大化中のウィンドウは `set_size` で最大化が解け(Windows では tao がそうします)、
最大化する前の位置へ戻ってしまうので、先に `outerPosition` を控えておき、`setPosition` でその左上へ置き直します。ウィンドウのサイズは `resize` のたびに `state.json` へ残るので、2 ペインで終了すれば
2 ペイン分の幅で起動します。
