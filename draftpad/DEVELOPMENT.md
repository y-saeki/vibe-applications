...
件数を書き込む `span` と「選択範囲」ボタンの 2 つだけで、あとは属性と `disabled` の付け外しです。
...
- **2 行 2 列のグリッド**。どこに何が来るかは `style.css` の `grid-area` だけで決めます。DOM の順序は
  ...
  3 個ぶんの幅です。各ボタンは自分より左にあるボタンの数だけ `margin-left` で右へ寄せ、検索行の 2 個は
  さらに 1 個ぶん先から始めるので、上下の行は右端で揃います。ウィンドウが狭いときは 1 列に畳み、
  そのときだけ検索行の 2 個も左端から始めます(揃える相手がいなくなるため)。
...
- **選択範囲だけの置換**。CodeMirror の置換は文書全体が相手で、範囲を渡す口がありません。渡せるのは
  `SearchQuery` の `test`——一致ごとに通すフィルタ——なので、選択範囲をそこに書いた同じ検索語のクエリを
  `setSearchQuery` で一時的に送り、CodeMirror の `replaceAll` を走らせてから元のクエリに戻します。
  置換文字列の扱い(`\n` などのエスケープ、正規表現の `$1` / `$&`)・取り消しが 1 回で済むこと・
  読み上げの通知は、すべて CodeMirror 側のコマンドに任せたままです。
