...
行アキは `.cm-mergeSpacer` の数と両ペインの同じ行の縦位置で確認します。開閉でウィンドウの大きさを変えないことは、
`set_size` が呼ばれていないことで確認します(ハーネスは `set_size` を知らないので、呼ばれればその `invoke` は失敗します)。
...
段の幅は「インデント幅」の設定です。行頭の空白を桁数で数え、タブ幅で割った商をその行の段数とします。CodeMirror が
...
バーを出すのはスクロールだけです。止めてから `IDLE_MS` 後にフェードアウトします。出るときは短く(120ms)、
消えるときは長く(480ms)、始まりと終わりをゆるめた曲線で薄くしていきます。動かした瞬間に見えてほしい一方で、
消える瞬間は目に留まらないほうがよいためです。出ている間に
...
開くときは、1 ペインの履歴も左ペインへ持ち越します。新しい `EditorState` には前の履歴がないので、`historyField.init` で
開く直前の値から始めます。左ペインで元に戻すと、開く前の編集までそのまま辿れます。右ペインは左のコピーで、履歴は空から
始まります。開いたこと自体は元に戻す対象にしません。開いたことを取り消すと右ペインの本文が消えるので、元に戻すを押しすぎた
だけで比較相手を失うことになるためです。

閉じたときは、残ったペインの履歴はそこから始まりますが、その先まで元に戻すと閉じたペインが戻ります
([#94](https://github.com/y-saeki/vibe-applications/issues/94))。`closePane` が閉じる直前の両ペインの `EditorState` を
控えておき、1 ペインの履歴が空で、文書が閉じた時点と同じなら、「元に戻す」は履歴を辿る代わりに 2 ペインを作り直します。
文書まで比べるのは、履歴が古い段を捨てる(既定で 100 段)ために、空になっても閉じた時点へ戻り切っていないことがあるからです。
作り直した各ペインも、開くときと同じく閉じた時点の履歴から始まるので、続けて元に戻すと閉じる前の編集を辿れます。
閉じた後に 1 ペインで元に戻した編集をやり直す分は、作り直しで失われます。控えは最後に閉じた 1 回分だけで、ペインを開くと
捨てます。

`Mod+Z` は `historyKeymap` より前に置いたキーで受けます。作り直しはそのキーを処理しているビュー自身を破棄するので、
処理が終わってから(microtask)行います。メニューと右クリックの「元に戻す」は `Editor.undo` から同じ処理に入り、
`canUndo` も戻せるペインがある間は true です。
...
検索・置換パネルは CodeMirror の bottom パネルで、エディタ要素の中、スクローラの下に付きます。
...
開いても閉じてもウィンドウの大きさは変えません。開くとウィンドウを左右半分ずつに分け、閉じると残ったペインが
ウィンドウ全体を使います。1 ペインのときより本文の幅が狭くなるので、左ペインの折り返し位置は開く前と変わります。

以前は、開くとウィンドウの幅を 2 倍に、閉じると半分にしていました。残るペインの幅が変わらず折り返し位置が動かない
利点はありましたが、フルスクリーンや最大化・ズーム中のウィンドウでの扱いに筋の通る形が見つからず、大きさを変える
途中の描画の乱れも抑えきれなかったため、やめました(#111)。

半分ずつをきっちり保つために、レイアウトから 2 つ外してあります。ペインの仕切りは `border` ではなく、バーの行と
...
gutter(`gutter: false`)ではなく、行自身の `box-shadow: inset` です。gutter は本文の横に列を足すので、
本文がペインの幅より狭くなります。`tests/e2e/compare.spec.ts` が、開いた後の両ペインの幅と本文の幅を見ています。
...
実装しています。Windows にはメニューバーがありません。どのキーに割り当てるかは環境設定で変えられます
(「キーボードショートカットの割り当て」を参照)。

元に戻す / やり直すは例外で、どちらのプラットフォームでも CodeMirror のキーマップが処理します。CodeMirror の
`historyKeymap` は「やり直す」の `Ctrl+Shift+Z` を Linux 向けにしか割り当てていないため、`src/editor-keymap.ts` で明示的に
...
件数を書き込む `span` 2 つと「選択範囲」ボタンだけで、あとは属性・`disabled`・フォーカスの移動です。

- **2 行 2 列のグリッド**。どこに何が来るかは `style.css` の `grid-area` だけで決めます。DOM の順序は
  CodeMirror が書いたまま、つまり Tab の順序もそのままです。ボタンは 5 つとも同じ幅で、右の列は置換行の
  3 個ぶんの幅です。各ボタンは自分の行で左にあるボタンの数だけ `margin-left` で右へ寄せるので、どちらの行も
  列の左端から始まり、「前へ」は「置換」の上、「次へ」は「選択範囲」の上に来ます。ウィンドウが狭いときは
  1 列に畳み、各行のボタンが自分の入力欄の下に落ちます。
- **パネルだけ一回り小さい**。入力欄 2 つ・ボタン 5 つ・トグル 2 つ・件数 2 つが 1 枚に載るので、本文と同じ
  文字サイズだと下書きの上で幅を取りすぎます。`font-size` はパネル自身に `--font-size-caption` で当て、
  高さは `--control-height-search` です。文字・入力欄・ボタン・件数がまとめて 1 段下がります。
...
- **置換した件数**。「すべて」と「選択範囲」が何件置換したかを、置換欄の右端に出します。数えるのは
  トランザクションの変更範囲の数で、どちらのボタンが走らせた置換でも同じ経路です(`userEvent` が
  `input.replace.all` のものを見ます)。1 回の置換と 1 つの下書きに紐づく数字なので、下書きか検索語が
  変わった時点で消します。
- **無効化**。一致が 0 件のあいだは「前へ / 次へ / 置換 / すべて」を `disabled` にします。「選択範囲」は
  条件が 1 つ多く、範囲が選択されていて、その中に一致が丸ごと 1 つ以上入っているときだけ押せます
  (半分だけ掛かった一致は置換しないので、それしか入っていない範囲では何も起きないままになります)。
  判定は数え上げ済みの一致リストへの包含で行います。範囲を引きずるたびに走るので、ここで数え直しはしません。
- **選択範囲だけの置換**。CodeMirror の置換コマンドは文書全体が相手で、範囲を渡す口がありません。
  `SearchQuery` の `test`(一致ごとに通すフィルタ)で絞る手もありますが、これは使えません——`\n` を含む
  検索語に割り当てられる複数行用カーソル(`MultilineRegExpCursor`)は、`test` が一致を弾いたときに読み取り
  位置を進めないため、同じ一致を見つけ直し、範囲より手前に一致が 1 つあるだけで走査が終わってしまいます
  (@codemirror/search 6.7.2 時点。単一行用のカーソルは進めるので、`\n` を含まない検索語では起きません)。
  そこで `SearchQuery.getCursor(state, from, to)` に範囲そのものを渡して一致を集め、置換後の文字列を組み立てて
  1 つのトランザクションで流し込みます。置換文字列の規則(`\n` などのエスケープ、正規表現の `$1` / `$&`)は
  CodeMirror の内側にあって取り出せないので、`src/search-panel.ts` に同じものを書いてあります。
- **置換後のフォーカス**。「置換 / 選択範囲 / すべて」を**押した**ときは、下書きにキーボードを戻します。
  直後の取り消しが、パネルに打った文字ではなく置換に掛かるようにするためです。移すのはクリックのときだけで、
  置換欄での Enter では移しません(移すと次の Enter が下書きの改行になります)。
...
これは見た目の好みではなく、CodeMirror が自前のスタッキング順を持っていることへの対策です。検索バー
(`.cm-panels`)は `z-index: 300`、ツールチップ(`.cm-tooltip`)は `z-index: 500` で、どちらも `.cm-editor` が自身のスタッキングコンテキストを作らないままページのルートに
...
パネルは「一般」と「ショートカット」の 2 タブです。見出しとタブは固定で、スクロールするのはその下の
`.panel-body` だけです。ショートカットタブの絞り込み欄と「すべて初期設定に戻す」も、スクロールの外に置いています
(`#preferences[data-tab]` で出し分けます)。スクロールバー(`src/overlay-scrollbar.ts`)は `.panel-body` に付け、
中身の `.panel-content` の大きさも監視します。`.panel-body` 自身の高さはタブを切り替えても絞り込んでも変わらない
ことがあり、それだけでは測り直しの合図になりません。

開くのは前回閉じたときのタブです(起動中だけ覚えます)。キーボードでは、タブの組が Tab の 1 か所で、左右の矢印で
切り替えます(WAI-ARIA のタブのパターン)。

### 真偽値のコントロール

環境設定の真偽値の項目(「空白文字を表示」など)はトグルとして描きます。パネルの設定は切り替えた瞬間に効くもので、
...
一覧と既定のキーは `src/shortcuts.ts` の 1 箇所にあります。変更できるもの(`SHORTCUTS`)と、見せるだけのもの
(`FIXED`)の 2 つです。環境設定のショートカットタブ・Windows のキー処理・macOS のメニューバー・CodeMirror の
キーマップは、どれもここから読みます。

変更できないものは、次のどちらかに当たるものです。

- クリップボード・すべてを選択・元に戻す / やり直す・カーソル移動・改行・削除・Tab。OS のどのテキスト欄でも
  同じに働くキーで、動かすと下書きを操作できなくなります
- macOS の定義済み項目(切り取り・コピー・貼り付け・すべてを選択・隠す・フルスクリーン)。muda はこれらの
  アクセラレータを差し替える手段を持ちません。フルスクリーンが Windows (F11) では変更でき、macOS では
  変更不可に並ぶのはこのためです

一覧にあるのは draftpad で実際に何かが起きるキーだけです。複数選択を無効にしているため、CodeMirror 標準の
「選択範囲の一致をすべて選択」(`Mod-Shift-l`)と「上 / 下にカーソルを追加」(`Mod-Alt-ArrowUp` / `Down`)は、
キーマップごと外しています。

### キーの表記

組み合わせは `src/keys.ts` の 1 つの形式で持ちます。修飾キーを `Ctrl` `Alt` `Shift` `Mod` の順に並べ、最後に
キーを `+` でつなぎます(`"Shift+Mod+V"` `"Alt+ArrowUp"` `"F11"`)。`Mod` は macOS では ⌘、それ以外では Ctrl です。
`Ctrl` は macOS の Control キーだけを指し、Windows では書きません。

Shift は必ず明示し、記号は Shift を押さない側の文字で書きます。`⇧⌘\` は `"Shift+Mod+"` で、`"Mod+|"` では
ありません。押されたキーから記号を決めるときは、CodeMirror と同じ `keyCode` の表(w3c-keyname の `base`)を
引きます。macOS の Option は L を ¬ に変えるので、文字そのものでは合わせられません。例外は Windows の
Ctrl+Alt で、これは AltGr と区別できないため、打たれた文字をそのまま使います(CodeMirror も同じ扱いです)。

登録できるのは、Shift 以外の修飾キーを含む組み合わせか、F1〜F12 です。文字キー単独や Shift+文字は入力なので
受け付けません。F13 以降を外しているのは、muda のアクセラレータが F12 までしか読めないためです。

同じ形式から、表示(`⇧⌘V` / `Ctrl+Shift+V`)、CodeMirror のキー名(`Shift-Mod-v`)、muda のアクセラレータ
(`CmdOrCtrl+Shift+V`)の 3 つを作ります。アクセラレータでは記号を名前で書きます(`Comma` `Backslash` など)。
muda は `+` で分割するためです。

### どこが処理するか

- アプリのコマンド(`src/commands.ts`)は、macOS ではメニューのアクセラレータ、Windows では `src/main.ts` の
  `keydown`(capture)が処理します
- エディタのコマンド(`src/editor-keymap.ts`)は CodeMirror のキーマップが処理します。キーマップは
  Compartment に入れてあり、割り当てが変わると差し替えます。CodeMirror 標準のキーマップからは、割り当てを
  変えられるものを取り除いてから、ユーザーのキーで付け直しています。取り除くときは、キーではなく実行する
  コマンド関数で見分けます。標準のキーマップは同じコマンドを `mac:` / `win:` の別表記で持つことがあり、キーで
  取り除くとそれを取りこぼすためです

CodeMirror の `searchKeymap` にある `Mod-f`(検索パネルを開く)も外しています。「検索・置換」はアプリのコマンドで、
メニューか `keydown` が先に処理します。CodeMirror 側に残すと、キーを移したあとも古いキーで開いてしまいます。

### macOS のメニューバー

起動時と割り当てが変わるたびに、ページが `set_menu_shortcuts` を呼び、メニュー項目の id ごとにアクセラレータ
(外したものは `null`)を渡します。Rust 側はメニューを作るときに該当する項目を `menu::Shortcuts` に取っておき、
`set_accelerator` で差し替えます。メニューは作り直しません。「編集」メニューから AppKit が足す項目を外す
`edit_menu::trim` は起動時に 1 度だけ走るもので、作り直すとそれもやり直しになるためです。

アクセラレータは 1 項目に 1 つしか持てません。アプリのコマンドにキーが 2 つある場合(パネルからは作れず、
`state.json` を直接書いた場合だけ)は、1 つ目だけがメニューに載ります。

ショートカットタブがキーの入力を待っている間は、全項目のアクセラレータを外します。メニューが持っているキーは
ページまで届かないため、外さないと ⌘Q を登録しようとした時点で終了してしまいます。入力を受け取るか、取り消すと
元に戻します。Windows では、環境設定パネルが `window` の `keydown` を `src/main.ts` より先に受け取って止めます。

### 重複

1 つのキーは 1 つのコマンドにしか割り当てられません。他のコマンドのキーを押したとき、↺ で戻そうとした既定の
キーが他のコマンドに移っていたときは、どちらも確認してから置き換えます。置き換えられた側からはそのキーが外れます。

### `state.json` の `shortcuts`

変更したコマンドだけを `{ id: キーの配列 }` で持ちます。空の配列は「キーを外した」という意味です。既定のままの
コマンドは項目を持たないので、後のバージョンで既定のキーを変えれば、触っていない人にはそれが届きます。

読むときは `src/shortcuts.ts` の `resolveBindings` が組み立てます。ユーザーが割り当てたキーは、後のバージョンで
別のコマンドの既定になっていても、ユーザーのものが優先します(既定の側から外します)。存在しない id、組み合わせと
して読めないもの、変更できないキー、重複したキーは、パネルから書かれることがないので捨てます。

### テスト

`tests/e2e/preferences.spec.ts` がショートカットタブ(変更・拒否・重複の確認・外す・↺・一括リセット・絞り込み)、
`tests/e2e/shortcuts.spec.ts` が Windows のキー処理とエディタのキーマップが新しいキーに従うこと、
`tests/e2e/menu.spec.ts` が `set_menu_shortcuts` に渡す内容を確かめます。メニューバーが実際にそのキーで反応するか
は webview の外なので、実機で確認します。
...
Tauri 公式・CodeMirror 公式・Microsoft 公式以外の依存は次の 1 つだけです。
...
draftpad/
  ...
  src/
    ...
    shortcuts.ts          ショートカットの一覧と既定のキー、state.json の割り当てからの組み立て
    keys.ts               キーの組み合わせの表記(キー入力から読む・表示・CodeMirror / muda の表記へ変換)
    editor-keymap.ts      エディタのキーマップ(CodeMirror 標準から、割り当てを変えられるものを付け直す)
    shortcut-list.ts      環境設定のショートカットタブ
  ...
  src-tauri/
    ...
    src/commands.rs       load_state / save_state / list_fonts / quit_app / show_context_menu / set_menu_shortcuts
