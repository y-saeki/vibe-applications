...
履歴は持ち越しません。新しい `EditorState` には前の履歴がないので、ペインを開いた時点で「元に戻す」はそこで止まります
(README の制限事項に書いてあります)。

閉じたときは、残ったペインの履歴もそこから始まりますが、その先まで元に戻すと閉じたペインが戻ります
...
作り直した各ペインは、`historyField.init` で閉じた時点の履歴から始まるので、続けて元に戻すと閉じる前の編集を辿れます。
