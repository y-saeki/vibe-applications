...
(`EditorView.lineWrapping`)。環境設定パネルはラベル列が固定で、入力欄の列が残りを取ります。この列は
`minmax(0, 1fr)` で、下限を持ちません。`1fr` だけだと下限が入力欄自身の min-content 幅——セレクトなら
一番長い選択肢の幅——になり、最小のウィンドウ幅ではそれが列より広くなって行が外へ出ます。
