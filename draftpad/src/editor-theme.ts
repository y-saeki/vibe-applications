// The look of the editor itself. What CodeMirror paints on its own — caret,
// selection, search matches, tooltips, and in the dark look the background and
// the syntax colors — is set here; the chrome around it is styled in style.css.
//
// Only the syntax colors are literals. The rest reads the custom properties
// style.css defines, so the palette has one home. The light look keeps
// CodeMirror's default syntax colors.
...
// The parts both looks share: the caret in the accent, the selection in its
// wash, search matches in the highlighter's yellow, and a dropped-open list of
// suggestions on the raised surface. CodeMirror's own defaults for these are
// keyed to the light and dark flag, so the rules are made once per flag.
const inkRules = {
...
const lightEditorTheme = EditorView.theme(inkRules, { dark: false })
...
export const lightTheme: Extension = [lightEditorTheme, syntaxHighlighting(defaultHighlightStyle)]
